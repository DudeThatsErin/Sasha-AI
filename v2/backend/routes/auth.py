"""
Authentication routes for Sasha AI v2
"""
...
async def reset_password(request: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    if not update_user_password(db, request.username, request.new_password):
...
async def update_theme(
    ...
    if not update_user_theme(db, current_user.username, request.theme):
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update theme")
